...
const (
	...
	dnsName     = serviceName + ".default.svc"                          // matches the MutatingWebhookConfiguration webhooksname
	defaultPort = 8000
)
...
var (
	port                = flag.Int("port", defaultPort, "port for the server. defaulted to 443 if unspecified ")
...
func StartServer() {
	...
	logger.Info("AlConstants:", "dns_name", dnsName, "port", *port, "certDir", certDir, "disableCertRotation", disableCertRotation, "dryRun", dryRun, "webhooks", webhooks)
